...
public class Voronoi{
    ...
    public func getEdges(v:[Point],w:Double, h:Double)->[Edge]{
        places = v;
        width = w;
        height = h;
        root = nil;
    ...
    private func insertParabola(p:Point){
        if let root = self.root{
            ...
            if(root.isLeaf && root.site.y - p.y < 1){
                ...
                let s = Point(x:(p.x + fp.x)/2,y: height);
                points.append(s);
                if(p.x > fp.x){
            ...
            let start = Point(x:p.x,y: getY(par.site, x: p.x));
            points.append(start);
    ...
    private func removeParabola(e:Event){
        let p1 = e.arch!;
        ...
        if(p0 === p2){
            println("ERROR RIGHT AND LEFT HAVE SAME CHILD!")
        }
        ...
        let p = Point(x: e.point.x, y: getY(p1.site, x: e.point.x));
        points.append(p);
    ...
    private func finishEdge(n:Parabola){
        if(n.isLeaf){
            return;
        }
        let mx:Double;
    ...
    private func getXOfEdge( par:Parabola,  y:Double)->Double{
    ...
    private func getParabolaByX(xx:Double)->Parabola{
        var par = root!;
        var x = 0.0;
        
        while(!par.isLeaf) // projdu stromem dokud nenarazÌm na vhodn˝ list
        {
            x = getXOfEdge(par, y: ly);
            if(x>xx){
                par = par.left
            }
            else{
    ...
    private func getY( p:Point,  x:Double)->Double{
        let dp = 2 * (p.y - ly);
        let a1 = 1 / dp;
        let b1 = -2 * p.x / dp;
    ...
    private func checkCircle(b:Parabola){
        let lp = Parabola.getLeftParent (b);
        let rp = Parabola.getRightParent(b);
        
        if let a  = Parabola.getLeftChild (lp), c  = Parabola.getRightChild(rp){
        
            if(a.site === c.site){
                return;
            }
        
            if let s = getEdgeIntersection(lp!.edge, b: rp!.edge){
                let dx = a.site.x - s.x;
                let dy = a.site.y - s.y;
